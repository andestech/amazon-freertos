...
#include "wifi/wilc1000/wireless_driver_extension/common/include/nm_common.h"
#include "wifi/wilc1000/wireless_driver_extension/driver/source/nmbus.h"
#include "wifi/wilc1000/wireless_driver_extension/driver/source/nmasic.h"
#include "wifi/wilc1000/wireless_driver_extension/driver/include/m2m_types.h"
...
sint8 firmware_download(void)
{
	...
	uint32 u32SecSize = 0;
	uint32 u32SecAddress = 0;
       	uint32 u32Val =0;
	uint32 size = 0;
	uint32_t pu8FirmwareUpperbound = 0;
	extern char __wifi_lmastart;
	extern char __wifi_lmaend;
#ifndef CFG_FLASHEXEC
...
#ifdef CFG_FLASHEXEC
	pu8FirmwareBuffer = (uint8_t*)((unsigned long)&__wifi_lmastart );
#else
	pu8FirmwareBuffer = (uint8_t*)((unsigned long)&__wifi_lmastart + (unsigned long)&__flash_start);
#endif
	size = (uint32)(&__wifi_lmaend - &__wifi_lmastart);
	pu8FirmwareUpperbound = (uint32_t)(pu8FirmwareBuffer + size);
	while((uint32_t)pu8FirmwareBuffer < pu8FirmwareUpperbound )
/*#else
